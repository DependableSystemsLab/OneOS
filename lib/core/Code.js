...
var FileSystemClient = require('./FileSystem.js').Client;
...
// characters being used by the application developer. A correct implementation should
...
// Hard-coded list here; find out if this list can be fetched programmatically
var BUILTIN_MODULES = [
  'assert',
  'buffer',
  'child_process',
...
function _getMemberExpressionProperty(node, index=0){
  if (node.type !== 'MemberExpression') return 0;

  let inner = _getMemberExpressionProperty(node.object, index);
  if (typeof inner === 'number'){
    if (inner === index) return node.property;
    else return inner + 1;  
  }
  else return inner;
  // return _getMemberExpressionProperty(node.object, index, depth+1);
}
...
AstHook.onEnter = {
  ...
  FunctionDeclaration: function(self, is_expression){
    ...
    // if (func_scope.params.length !== 0 
    //     || Object.keys(func_scope.declared).length !== 0
    //     || Object.keys(func_scope.hoisted).length !== 0
    //     || Object.keys(func_scope.props).length !== 0){
    ...
    if (!is_expression){
      // If func_scope.func_name is not in self.scope.hoisted, 
      // it implies function was declared inside an if block or for block
  ...
  CallExpression: function(self){
    ...
    if (self.node.callee.type === 'MemberExpression'){
      ...
      else if (self.node.callee.object.name === 'process'
        && (self.node.callee.property.name === 'input'
          || self.node.callee.property.name === 'output'
          || self.node.callee.property.name === 'getActor')){

        self.scope.meta[self.node.callee.property.name].push(self.node.arguments[0].value);
      }
      else if (self.node.callee.object.type === 'MemberExpression'){

        if (self.node.callee.object.object.name === 'process'
            && self.node.callee.object.property.name === 'locale'){

          if (self.node.callee.property.name === 'inputDevice'){
            self.scope.meta['localInputDevice'].push(self.node.arguments[0].value);
          }
          else if (self.node.callee.property.name === 'outputDevice'){
            self.scope.meta['localOutputDevice'].push(self.node.arguments[0].value);
          }
        }
      ...
      if (['setImmediate', 'setTimeout', 'setInterval', 
				 'clearImmediate', 'clearTimeout', 'clearInterval'].indexOf(self.node.callee.name) > -1){
        var replace = esprima.parse(SCOPE_PREFIX+'.'+self.node.callee.name).body[0].expression;
        self.node.callee = replace;
      }
      else if (self.node.callee.name === 'require'){
        var replace = esprima.parse(SCOPE_PREFIX+'.'+self.node.callee.name).body[0].expression;
        self.node.callee = replace;

        // caveat: cannot handle dynamic requires, because we have to evaluate the argument
        if (self.node.arguments[0].type === 'Literal'){
          var package_name = self.node.arguments[0].value.split('/')[0];
          if (!BUILTIN_MODULES.includes(package_name)){
            self.scope.meta['dependencies'].push(package_name);
          }
        }
      }
  ...
  AssignmentExpression: function(self){
    ...
    if (self.node.left.type === 'MemberExpression'
      && _getMemberExpressionObject(self.node.left.object).type === 'ThisExpression'){
      var prop = _getMemberExpressionProperty(self.node.left);
      // console.log(prop);
      self.scope.props[prop.name] = prop.name;
    }
 ...
 */
function Code(name, raw_code){
  if (!(this instanceof Code)) return new Code(name, raw_code);
  ...
  this.source = Code.instrument(name, raw_code);
...
//     if (comments[i].type === 'Block' && comments[i].value.indexOf('oneos.meta') === 0){
...
Code.readMetadata = function(raw_code){
  ...
  for (var i=0; i < ast.comments.length; i++){
    if (ast.comments[i].type === 'Block' && ast.comments[i].value.indexOf('oneos.meta') === 0){
      var body = ast.comments[i].value.split('\n').slice(1,-1).join('\n');
      meta = yaml.safeLoad(body);
      break;
    }
  }
  meta = Object.assign(meta, hook.scope.meta);
 ...
 */
Code.instrument = function(code_name, raw_code){
  // TODO: revise the function signature to determine whether to keep `code_name` argument
  ...
  // console.log(hook.scope.meta);

  // Process metadata about the program
  // var meta = JSON.stringify(Code._extractMeta(ast.comments));
  var meta = JSON.stringify({});

  // Prepare the oneos template
  var template_str = `require('oneos').bootstrap(module, function(${SCOPE_PREFIX}){}, '${code_name}', ${meta});`;
...
Code.restore = function(snapshot){
  ...
  return `require('oneos').bootstrap(module, 
		function(${SCOPE_PREFIX}){ ${content} },
		'${snapshot.meta.code_name}/${snapshot.meta.instance_id}',
		${meta});`;
};
 ...
 */
Code.fromString = function(code_name, raw_code){
  return new Code(code_name, raw_code);
};
 ...
 */
Code.fromFile = function(file_path){
  return new Code(path.basename(file_path), fs.readFileSync(file_path).toString());
};
...
Code.fromSnapshot = function(snapshot){
  ...
  code.source = Code.restore(snapshot);
...
/** Determines if the given function is a closed lambda function */
Code.isLambda = function(code){
  // to implement
}
...
var NATIVE_TRANSFORMERS = {
  ...
  'Pubsub': {
    makeSerializable: function(item){
      ...
      // TODO: Complete implementation of Pubsub serialization
      // 
  ...
  'FileSystemClient': {
    ...
    makeCode: function(item){
      ...
      //   return '"'+topic+'":'+makeCode(item.value.handlers[topic]);
      // }).join(',')+'}';
      return 'new '+SCOPE_PREFIX+'.FileSystemClient()';
    }
  },
  'oneos.reference': {
    makeCode: function(item){
...
makeSerializable['object'] = function(item){
  ...
  // console.log('Serializing '+item.constructor.name, item);
  return {
...
function RootScope(local_module, code_uri, user_meta){
  if (!(this instanceof RootScope)) return new RootScope(local_module, code_uri, user_meta);
...
RootScope.prototype.FileSystemClient = FileSystemClient;
...
/* This is an "Agent-space" function, and it is the first thing
called inside the userland child process. */
Code.bootstrap = function(local_module, main, code_uri, user_meta){
  // console.log(local_module);

  // inject some environment:
  // console.log('[Code] User Process Environment variables ', process.env);
	
  let rootScope = new RootScope(local_module, code_uri, user_meta);

  let _requests = {};
  function ipcRequest(verb, payload){
    return new Promise((resolve, reject)=>{
      ...
      // console.log('Making IPC Request '+request_id+' : '+verb);
    });
  }

  if (process.connected){
    // if this block is executed, we assume that the `process` object
    // has already been augmented via the `./vm.js` and the `process.oneos`
    // property points to an object containing metadata.
    // 
    // the following code further augments the `process` object,
    // attaching the following properties to it:
    //   `api`: an object exposing the "root-level" APIs to the user process
    //   `input`: a function to instantiate an input stream for the process
    //   `output`: a function to instantiate an output stream for the process
    //   `getActor`: a function to instantiate an Actor object
    //   `locale`: a proxy object to access local resources such as I/O devices
    
    let pubsub = new Pubsub.IPCChild(process.oneos.pubsub_ipc, process.oneos.pubsub_url);
    let streams = {
      output: {},
      input: {},
      _localInput: {},
    ...
    process.api = {
      'getAllRuntimes': (...args)=>ipcRequest('getAllRuntimes', args),
      'getAllAgents': (...args)=>ipcRequest('getAllAgents', args),
      'getAllPipes': (...args)=>ipcRequest('getAllPipes', args),
      'getRuntime': (...args)=>ipcRequest('getRuntime', args),
      'getAgent': (...args)=>ipcRequest('getAgent', args),
      // 'getAgents': ()=>ipcRequest('getAgents')
    };

    process.locale = {
      'inputDevice': (name)=>{
        if (!(name in streams._localInput)) streams._localInput[name] = pubsub.getInputStream(process.oneos.runtimeID+'-input-'+name+':out');
        // console.log('Input Device: '+name);
        ipcRequest('getLocalInputDevice', [ name ])
          .then((resp)=>{
            // console.log(resp);
          }, (err)=>{
            console.log(err);
        ...
        return streams._localInput[name];
      },
      'outputDevice': (name)=>{
        if (!(name in streams._localOutput)) streams._localOutput[name] = pubsub.getOutputStream(process.oneos.runtimeID+'-output-'+name+':in');
        // console.log('Output Device: '+name);
        ipcRequest('getLocalOutputDevice', [ name ])
          .then((resp)=>{
            // console.log(resp);
          }, (err)=>{
            console.log(err);
        ...
        return streams._localOutput[name];
      }
    };

    process.input = (channel, content_type)=>{
      if (!(channel in streams.input)) streams.input[channel] = pubsub.getInputStream(process.oneos.agentID+':'+channel, content_type);
      console.log('Agent '+process.oneos.agentID+' using Input '+channel);
      return streams.input[channel];
    }

    process.output = (channel, content_type)=>{
      if (!(channel in streams.output)) streams.output[channel] = pubsub.getOutputStream(process.oneos.agentID+':'+channel, content_type);
      console.log('Agent '+process.oneos.agentID+' using Output '+channel);
      return streams.output[channel];
    }

    process.getActor = pubsub.getActor.bind(pubsub);

    process.on('message', function(message){
      // console.log('Parent process says:', message);
      if (message.response_id){
        if (message.response_id in _requests){
          // console.log('Resolving IPC Request', message.payload);
          clearTimeout(_requests[message.response_id].timeout);
          (message.error ? _requests[message.response_id].reject(new Error(message.payload))
            : _requests[message.response_id].resolve(message.payload));
          delete _requests[message.response_id];
          // console.log('... resolved', _requests);
        }
      }
      else if (message.verb && message.request_id){
      	if (message.verb in RuntimeHandler){
  			var result = RuntimeHandler[message.verb](rootScope, message.payload);
  			// console.log('Ran IPC Command ', message);
  			process.send({ 
  			...
  			process.send({ response_id: message.request_id, error: 'UnknownCtrl', message: message });
  		}
      }
    });

    // for now, exit when it becomes zombie.
    // Later we would need state persistence
    process.on('disconnect', ()=>{
      console.log('Lost Parent Process!!!!! Exiting');
      process.exit(1);
    });
  }
  else {
    throw new Error('Not connected to the Runtime process, exiting');
  }
