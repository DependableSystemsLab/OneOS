...
Code.bootstrap = function(local_module, main, code_uri, user_meta){
  ...
  if (process.connected){
    ...
    process.input = (channel)=>{
      if (!(channel in streams.input)) streams.input[channel] = pubsub.getInputStream(process.oneos.agentID+':'+channel);
      console.log('Agent '+process.oneos.agentID+' using Input '+channel);
      return streams.input[channel];
    }

    process.output = (channel)=>{
      if (!(channel in streams.output)) streams.output[channel] = pubsub.getOutputStream(process.oneos.agentID+':'+channel);
