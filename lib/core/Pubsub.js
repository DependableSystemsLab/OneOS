...
class InputStream extends Readable {
  // constructor (pubsub, topic, deserializer=DESERIALIZERS['buffer']) {
  constructor (pubsub, topic, content_type='buffer') {
  ...
  constructor (pubsub, id){
    ...
    ipc.serve(id+'.ipc.sock', ()=>{
      ...
      ipc.server.on('subscribe', (data, socket)=>{
        ...
        let handler = (message)=>{
          console.log('Sending IPCChild '+id+' data for '+data.topic);
        ...
        this.pubsub.subscribe(data.topic, handler, 'buffer')
          .then((ref)=>{
