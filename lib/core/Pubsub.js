...
class InputStream extends Readable {
  constructor (pubsub, topic, deserializer=DEFAULT_DESERIALIZER) {
    super();
    this.pubsub = pubsub;
    this.topic = topic;
    ...
    let handler = (message, rawData)=>{
      ...
      // console.log(chalk.yellow(this.pubsub.id)+chalk.green(' <'+topic+'> : ')+ ' got '+(typeof data)+' '+Object.getPrototypeOf(message).constructor.name);
      // console.log(String(message).substring(0, 50));
...
class OutputStream extends Writable{
  ...
  _write (chunk, encoding, callback){
    ...
    // console.log(String(chunk).substring(0,100));
    this.pubsub
      // .publish(this.topic, this.serializer(chunk))
      .publish(this.topic, chunk)
      .then(
        (result)=>{
          callback(null);
      ...
      this._requests[request_id] = {
        ...
        timeout: setTimeout(()=>{
          if (request_id in this._requests){
            reject(new Error('PubsubRequestTimeout to '+target_topic+' {'+verb+'}'));
  ...
  constructor (url, options){
    ...
    this.ready = new Promise((resolve, reject)=>{
      this.client.on('connect', (ack)=>{
        console.log(Date.now()+chalk.green(' Pubsub '+this.id)+' connected');
        resolve(this.client);
      });
  ...
  constructor (pubsub, id){
    ...
    ipc.serve(id+'.ipc.sock', ()=>{
      ...
      ipc.server.on('subscribe', (data, socket)=>{
        ...
        this.pubsub.subscribe(data.topic, handler);
      });
      ...
      ipc.server.on('publish', (data, socket)=>{
        ...
        this.pubsub.publish(data.topic, data.message);
      });
