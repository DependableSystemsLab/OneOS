...
Scheduler.API = {
	'run': (self, payload, response) => {
      ...
      let language = EXT_TO_LANG[path.extname(payload.source_path)] || 'unknown';
    ...
    if (payload === '*'){
      return self.systemAPI.getAllPipes()
        .then((pipes)=>Promise.all(Object.values(pipes).map((pipe)=>self.pubsub.request(pipe.runtime, 'kill_pipe', pipe.id))))
        .then(()=>self.systemAPI.getAllAgents(true))
