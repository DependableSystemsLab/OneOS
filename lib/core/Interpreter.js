...
function parse(input) {
  ...
  function maybe_binary(left, my_prec) {
    ...
    if (tok) {
      ...
      if (his_prec > my_prec) {
        ...
        var binary = {
          type: tok.value == '=' ? 'assign':'BinaryExpression',
          operator: tok.value,
          left: left,
          right: right
        };
  ...
  function parse_atom() {
    ...
    //     ()=>{
          ...
          if (is_op('$')) return parse_variable();
...
function evaluate(exp, env) {
  ...
  case 'VariableBinding':
    ...
    .then(result => {
      console.log('Variable Binding: ' + exp.name + ' = ' + String(result));
      env.set(exp.name, result);
      return result;
    });
  ...
  case 'BinaryExpression':
    return Promise.all([ evaluate(exp.left, env), evaluate(exp.right, env) ])
      .then(results => apply_op(exp.operator, results[0], results[1]));

    /*apply_op(exp.operator,
    ...
    return evaluate(exp.func, env)
      .then((func)=>{
        if (typeof func === 'function'){
          ...
          // if file is oneos script, interpret it
          if (extname === '.osh')
          {
            return new Promise((resolve, reject) => {
              // read file content
              env.api.fs.readFile(func.path, (err, data) => {
                if (err) reject(err);
                else {
                  console.log('Executing OneOS Script ' + func.path);
                  evaluate(compile('{' + String(data) + '}'), env).then(resolve, reject);
                }
              });
            });
          }
