...
const DEBUG = false;  // setting this to true will suppress the output on the web dashboard
...
const SERVICE_CODE_DIRECTORY = path.resolve(__dirname, './runtime/services');
const SERVICE_AGENTS = {
  'local-input': path.join(SERVICE_CODE_DIRECTORY, 'local-input.js'),
  'local-output': path.join(SERVICE_CODE_DIRECTORY, 'local-output.js')
}
...
class Runtime extends EventEmitter {
  ...
  // This function will be called upon request by a child Agent.
  // If the runtime indeed has the requested device,
  //     it returns the Promise resolving to the ID of the singleton proxy object, 
  //     which the child Agent eventually interacts with.
  //     If not already instantiated, instantiate the singleton Input object
  //         using the provided driver name, and then return a Promise resolving
  //         to the ID of the proxy object
  // If the runtime does not have the requested device,
  //     it rejects with an Error
  getLocalInputDevice (name){
    if (name in this.config.io.input){

      if (name in this.io.input){
        return Promise.resolve(this.io.input[name].config.id);
      }
      else {
        return readFile(SERVICE_AGENTS['local-input'])
          .then((source)=>{
            this.io.input[name] = new Agent(this, {
              // pubsub_url: this.pubsub.url,
              id: this.config.id+'-input-'+name,
              name: 'input-'+name,
            ...
            return this.io.input[name].start({ args: [ this.config.io.input[name] ] });
          })
          .then((agent)=>agent.config.id);
      }
      // return LocalDevice.getInput(this.config.io.input[name]);
    }
    else return Promise.reject(new Error('Local Input Device ['+name+'] is not available'));
  }

  getLocalOutputDevice (name){
    if (name in this.config.io.output){

      if (name in this.io.output){
        return Promise.resolve(this.io.output[name].config.id);
      }
      else {
        return readFile(SERVICE_AGENTS['local-output'])
          .then((source)=>{
            this.io.output[name] = new Agent(this, {
              // pubsub_url: this.pubsub.url,
              id: this.config.id+'-output-'+name,
              name: 'output-'+name,
              source: source.toString()
            }, {
              isDaemon: false,
            ...
            return this.io.output[name].start({ args: [ this.config.io.output[name] ] });
          })
          .then((agent)=>agent.config.id);
      }
      // return LocalDevice.getInput(this.config.io.input[name]);
    }
    else return Promise.reject(new Error('Local Output Device ['+name+'] is not available'));
  }
...
Runtime.DEFAULT_CONFIG = {
  ...
  'store_url': 'mongodb://localhost:27017/oneos-fs',
  'id': uuid(),
...
Runtime.Operations = {
  'start_agent': (self, payload, response)=>{
    ...
    // 	.then((source)=>{
    		let agent_id = payload.name + '.' + Agent.nextId();
...
class DummyRuntime {
  constructor (config, options){
    this.config = Object.assign(JSON.parse(JSON.stringify(Runtime.DEFAULT_CONFIG)), config);
    this.pubsub = {
      url: 'mqtt://localhost'
    }
  }
