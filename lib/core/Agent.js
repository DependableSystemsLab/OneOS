...
class Agent extends EventEmitter {
  constructor (runtime, config, options){
    ...
    if (this.config.language === 'javascript'
      ...
      this.config.dependencies = meta.dependencies;
      ...
      this.dependencyReady = helpers.ensurePackages(this.config.dependencies);
      ...
      this.dependencyReady = Promise.resolve(true);
    }
  ...
  summary (){
    return {
      ...
      channels: [
        { name: 'stdin', type: 'input', content_type: this.io.stdin.content_type, stat: this.io.stdin.stat },
        { name: 'stdout', type: 'output', content_type: this.io.stdout.content_type, stat: this.io.stdout.stat },
        { name: 'stderr', type: 'output', content_type: this.io.stderr.content_type, stat: this.io.stderr.stat }
      ],
      status: this.status,
      // gps: { lat: Math.random()*160 - 80, long: Math.random()*360 - 180 },
      gps: this.runtime.gps,
      stat: this.stat
    };
  }
  ...
  start (options = {}){
    ...
    // return this.pubsub.ready.then(()=>{
      ...
      if (this.config.language === 'javascript'
		    ...
		    console.log(Date.now()+' '+chalk.red('Agent '+this.config.id+' STARTING')+' args : '+(options.args && options.args.join(', ')));
        ...
        this.proc.send({
          ...
          store_url: this.runtime.config.store_url,
      ...
      {
        console.log('Cannot start an Agent written in "' + this.config.language + '" language. No language runtime found.');
      }
