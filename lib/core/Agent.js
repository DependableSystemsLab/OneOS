...
class Agent extends EventEmitter {
  constructor (runtime, config, options){
    ...
    this.io = {
      'stdin': this.pubsub.getInputStream(this.config.id+':stdin', 'text'),
      'stdout': this.pubsub.getOutputStream(this.config.id+':stdout', 'text'),
      'stderr': this.pubsub.getOutputStream(this.config.id+':stderr', 'text')
    };
